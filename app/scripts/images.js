...
const getAllImageFileNames = (path, imageFiles, subDirectory = '') => {
	const files = fs.readdirSync(path, { withFileTypes: true }).filter((file) => {
		if (file.name.includes('@2x') || file.name.includes('@3x')) return false;
		return (
	...
	files.forEach((file) => {
		if (file.isDirectory()) {
			...
			imageFiles.push({
				path: subDirectory != '' ? `${subDirectory}/${file.name}` : file.name,
				name: file.name,
			});
		}
	});
...
const generate = () => {
	getAllImageFileNames('app/res/images', imageFileNames);
...
`;
	...
	fs.writeFileSync('app/res/images.ts', string, 'utf8');
};
